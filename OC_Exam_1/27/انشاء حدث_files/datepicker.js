...
var DatepickerObject = function (el, extraOptions) {
	var Datepicker;
	this.datepickerList = {};

	var me = this;
	
    var element = el ? "#" + $(el).attr("id") || "." + $(el).attr("class") : '.datepicker-here';
	extraOptions = extraOptions || {};
	
    (function (window, $, undefined) {
        var pluginName = 'datepicker',
            autoInitSelector = element,
        ...
        Datepicker = function (el, options) {
            ...
            this.opts = $.extend(true, {}, defaults, options, extraOptions, this.$el.data());
            ...
            this.currentDate = this.opts.currentDate ? new Date(this.opts.currentDate) : this.opts.startDate;
        ...
        datepicker.prototype = {
            ...
            init: function () {
				...
				var dp = new Datepicker.Body(this, this.currentView, this.opts);
				me.datepickerList[$(this.el).attr("id")] = dp;
                this.views[this.currentView] = dp;
                ...
                if (me instanceof DatepickerObject) {
                	var formattedDate = this.formatDate(this.loc.dateFormat, this.currentDate);
                	me.setValue(formattedDate, $(this.el).attr("id"));
                }
            },
            ...
            selectDate: function (date) {
                ...
                if (opts.onSelect) {
					var formattedDate = _this.formatDate(_this.loc.dateFormat, date);
                    _this.opts.onSelect(formattedDate, date, this);
            ...
            _setInputValue: function () {
                var _this = this,
                    opts = _this.opts,
                    format = "dd/mm/yyyy", //_this.loc.dateFormat,
        ...
        $(function () {
            $(autoInitSelector).datepicker(extraOptions);
        })
    ...
    ; (function () {
        var template = '' +
            '<div class="datepicker--nav-action prev" data-action="prev">#{prevHtml}</div>' +
            '<div class="datepicker--nav-title">#{title}</div>' +
            '<div class="datepicker--nav-action next" data-action="next">#{nextHtml}</div>',
...
DatepickerObject.prototype.setValue = function (currentDate, elementID) {
	if (currentDate) {		
		var currentInputObject = this.datepickerList[elementID];
		
		if (currentInputObject) {
			var currentDateObject = new Date(currentDate);
			var d = currentDateObject.getDate();
			var m = currentDateObject.getMonth();
			var y = currentDateObject.getFullYear();

			var input = currentInputObject.d.el;
			var cells = currentInputObject.$cells
			
			var currentDateContainer = cells.find(".datepicker--cell[data-date='" + d + "'][data-month='" + m + "'][data-year='" + y + "']");
			if (currentDateContainer && currentDateContainer.length > 0) {
				currentDateContainer.click();
				
				var dateString = createDateStringFromDateObject(currentDateObject);
