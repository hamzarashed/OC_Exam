...
function sharePresentation() {
  ...
  sharePresentationDialog = window.open(
...
function shareDesktop() {
  ...
  shareDesktopDialog = window.open(
    shareDesktopUrl,
    "sharegplus",
    "loaction=no,titlebar=no,modal=yes,height=" +
      (height - y * 2) +
      ",width=" +
      (width - x * 2) +
      ",left=" +
...
function loadMeetingList() {
  ...
  for (var index in meetings) {
    ...
    meetingTemplate = meetingTemplate
      .replace("[title]", fixTextDecode(meeting.meetingTopic))
      .replace(
        "[date]",
        String.format(
          "{0} {1}",
          meeting.meetingStartDate,
          fixTextDecode(meeting.meetingStartMonth)
        )
      )
      .replace(
        "[time]",
        String.format(
          "{0}:{1}",
          meeting.meetingStartHour,
          meeting.meetingStartMinutes
        )
      );
  ...
  $(".cancelMeeting").click(function () {
    ...
    if (topic == "تقييم سير العمل" && !localStorage.getItem("26")) {
      localStorage.setItem("26", 1);
      alert("تم ارسال سؤالك سيتم نقلك للسؤال 27");
...
(function () {
  ...
  this.OnlineMeeting.dataManager = {
    ...
    setPresentationBackground: function () {
      $(".snapshot").attr(
        "src",
        getResourceActualPath(currentMeeting.slideSnapshot)
      );
    },
    populateMeetingAttendeesList: function () {
      ...
      if (meetingParticipants != null) {
        ...
        $.each(meetingParticipants, function (index, item) {
          var pictureUrl = getResourceActualPath(
            item && item.gender == 2
              ? "../img/female_user.png"
              : "../img/male_user.png"
          );
          var statusrl = getResourceActualPath(
            item && item.status ? "../img/online.png" : "../img/offline.png"
          );

          var participant = String.format(
            templateManager.meetingParticipant,
            pictureUrl,
            item.name,
            statusrl,
            item.status ? "Online" : "Offline"
          );
          $("<tr></tr>").html(participant).appendTo(participantTable);
        });
      }
    },
    ...
    postModifiedMeetingInfo: function (meetingTopic) {
      ...
      if (meeting) {
        $.post(
          String.format("{0}?{1}", editMeetingUrl, $.param(meeting)),
          null
        );
        window.location.href = allMeetingsUrl;
      }
    ...
    postCancelledMeetingInfo: function (meetingTopic) {
      ...
      if (meeting) {
        ...
        $.post(
          String.format("{0}?{1}", allMeetingsUrl, $.param(meeting)),
          null
        );
        window.location.href = allMeetingsUrl;
      }
